...

{
	...
	if (coeff_detector_total > 0.01f)
	{
		...
		float thetaDetector = acos((zDetectorCrystalCenter - zImage) / distance);
		float phiDetector = atan2(yDetectorCrystalCenter - yImage, xDetectorCrystalCenter - xImage);
		...
		for (unsigned int id_Detector = 0; id_Detector < numDetectorbins; id_Detector++)
		{
			if (id_Detector != idxDetector)
			{
				...
				float crit_other_radius = sqrt(deviceparameter_Detector[id_Detector * 12 + 4] * deviceparameter_Detector[id_Detector * 12 + 4] + deviceparameter_Detector[id_Detector * 12 + 5] * deviceparameter_Detector[id_Detector * 12 + 5] + deviceparameter_Detector[id_Detector * 12 + 6] * deviceparameter_Detector[id_Detector * 12 + 6]);
				...
				crit_other_radius = crit_other_radius / distance_other/2.0;

				float thetaDetector_other = acos((zDetector_other - zImage) / distance_other);
				float phiDetector_other = atan2(yDetector_other - yImage, xDetector_other - xImage);

				float cosValue = cos(thetaDetector) * cos(thetaDetector_other) * cos(phiDetector - phiDetector_other) + sin(thetaDetector) * sin(thetaDetector_other);
				cosValue = fminf(fmaxf(cosValue, -1.0f), 1.0f); // ȷ��cosValue��[-1,1]��Χ��
				float distance_sphere = acos(cosValue);

				if (distance_sphere <= (asin(crit_other_radius) + asin(crit_self_radius))*3.0f)
				{
		...
		{	
			...
			for (unsigned int idxCollimator_inlayer = 0; idxCollimator_inlayer < numCollimatorHoles[m]; idxCollimator_inlayer++)
			{
				...
				float distance_other = sqrt((xImage - x_cylinder) * (xImage - x_cylinder) + (yImage - (y1_cylinder + y2_cylinder) / 2.0f) * (yImage - (y1_cylinder + y2_cylinder) / 2.0f) + (zImage - z_cylinder) * (zImage - z_cylinder));
				crit_other_radius = crit_other_radius / distance_other;

				float thetaHole = acos((z_cylinder - zImage) / distance_other);
				float phiHole = atan2((y1_cylinder + y2_cylinder) / 2.0f - yImage, x_cylinder - xImage);

				float cosValue = cos(thetaDetector) * cos(thetaHole) * cos(phiDetector - phiHole) + sin(thetaDetector) * sin(thetaHole);
				cosValue = fminf(fmaxf(cosValue, -1.0f), 1.0f); // ȷ��cosValue��[-1,1]��Χ��
				float distance_sphere = acos(cosValue);
		...
		for (unsigned int NumZ = 0; NumZ < divideZ; NumZ++)
		{
			for (unsigned int NumX = 0; NumX < divideX; NumX++)
			{
				for (unsigned int NumY = 0; NumY < divideY; NumY++)
				{
					...
					float COSangle = (yDetector - yImage) * rsqrt(distancesq);
					
					/////////////////////////////////  Solid Angle ////////////////////////////////
					float solid_angle_x = (thicknessDetectorCrystal * heightDetectorCrystal / (float)divideZ / (float)divideY) / (4 * M_PI * distancesq) * fabs((xDetector - xImage) * cos(rotationAngel_DetectorCrystal) - (zDetector - zImage) * sin(rotationAngel_DetectorCrystal)) * rsqrt(distancesq);
					float solid_angle_z = (widthDetectorCrystal * thicknessDetectorCrystal / (float)divideX / (float)divideY) / (4 * M_PI * distancesq) * fabs((xDetector - xImage) * sin(rotationAngel_DetectorCrystal) + (zDetector - zImage) * cos(rotationAngel_DetectorCrystal)) * rsqrt(distancesq);
					float solid_angle_y = (widthDetectorCrystal * heightDetectorCrystal / (float)divideX / (float)divideZ) / (4 * M_PI * distancesq) * fabs(yDetector - yImage) * rsqrt(distancesq);
					...
					float x_tmp = xDetector_self + dist_extend * (xDetector_self - xImage_self) * rsqrt(distancesq);
					float y_tmp = yDetector_self + dist_extend * (yDetector_self - yImage_self) * rsqrt(distancesq);
					float z_tmp = zDetector_self + dist_extend * (zDetector_self - zImage_self) * rsqrt(distancesq);
					...
					dst[dstIndex] += detection_eff_each * solid_angle * absorption_factor;
				}
			}
		}
	}
...
int test1(float* parameter_Collimator, float* parameter_Detector, float* parameter_Image, float* dst)
{

	cout << "Get into test1" << endl;
	...
	cudaSetDevice(2); // Set device 0 as current
	cout << "Set Device to Device 2" << endl;
	...
	/*ÿ������Grid��������ഴ��65535���߳̿飬ÿ���߳̿飨Block��һ�������Դ���512�������߳�*/
	//ȡ16*32�߳̿�
	...
	photodetectorCudaMe <<<gridSize, blockSize >>> (
		...
		numProjectionSingle,
		numImagebin);
	//cudaThreadSynchronize();//cpu��gpu��ͬ������cpu�ȴ�gpu�������
