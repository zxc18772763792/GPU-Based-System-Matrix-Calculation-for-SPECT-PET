...
int main(int argc, char* argv[])
{
	...
	for (int i = 1; i < argc; ++i)
	{
		...
		{
			cout << "Usage: " << argv[0] << " [-PE PE_SysMat_path] [-GeoCollimator GeometryRelationShip_Collimator2Crystal_path] [-cuda GPU_ID[,GPU_ID...]]" << endl;
			return 0;
		}
		else
		{
	...
	for (size_t idxRotation = 0; idxRotation < numRotation; idxRotation++)
	{
		...
		const bool generatingGeometry = ((int)floor(parameter_Physics[9]) == 1);
		...
		if (generatingGeometry) parameter_Physics[9] = 0;
		for (size_t worker = 0; worker < workerCount; ++worker)
		{
			if (results[worker] < 0)
			{
